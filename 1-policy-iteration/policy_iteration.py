...
class PolicyIteration:
    ...
    def policy_evaluation(self):
        ...
        # 모든 상태에 대해서 벨만 기대방정식을 계산 (좌표:(0,0), (0,1), ...)
    ...
    def get_action(self, state):
        policy = self.get_policy(state) #확률로 들어가 있을테고?
        policy = np.array(policy)

        #가장 큰 수가 2개이면, 2개중 랜덤하게 하나의 인덱스 리턴
        return np.random.choice(4, 1, p=policy)[0] #개중에서 1개를 선택하되 0번쨰 인덱스
